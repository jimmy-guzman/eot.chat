...
test("should complete the full chat room happy path", async ({ browser }) => {
  ...
  // Step 4: Send a plain text message → renders in the message area
  await page1.getByPlaceholder("Send anything...").fill("Hello world");
  await page1.getByRole("button", { name: "Send" }).click();
  await expect(page1.getByText("Hello world")).toBeVisible();

  // Step 5: Open a second browser context at the same URL → displayName prompt shown
  ...
  // Step 6: Second context enters display name and joins → first context sees updated participant list
  ...
  // Step 7: Second context sends a message → first context receives and renders it
  ...
  // Step 8: Second context clicks Exit Room → first context sees the participant leave
  ...
  // Step 9: First context clicks Exit Room → navigating back to /r/<id> redirects to /
